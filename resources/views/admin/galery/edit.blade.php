                        ...
                        <form action="{{ route('galery.update', $galery->id) }}" method="post"
                            ...
                            <div class="mb-3">
                                ...
                                <textarea type="text" name="description" class="form-control"
                                    id="description">{{ old('description', $galery->description) }}</textarea>
                                @error('description')
                                    <span class="text-danger mt-2">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="mb-3">
                                <label for="gambar" class="form-label">{{ __('Current Image') }}</label><span
                                    aria-hidden="true" role="presentation" class="field_required"
                                    style="color:#ee0000;">*</span>
                                <div class="row">
                                    @foreach($galery->images as $key => $photo)
                                        <div class="col-md-">
                                            <button type="button" class="btn btn-link"
                                                ...
                                                <img src="{{ asset('uploads/galeri/' . $photo->image) }}"
                                                    width="140">
                                            </button>
                                        </div>
                                    @endforeach
                            ...
                            <div class="mb-3">
                                ...
                                <input type="file" name="new_images[]" class="form-control-file" id="new_images"
                                    multiple>
                                @error('new_images')
                                    <span class="text-danger mt-2">{{ $message }}</span>
                                @enderror
